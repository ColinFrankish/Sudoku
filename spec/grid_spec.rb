...
describe 'Grid' do
  ...
  it "should be able to know what is in the first box" do
      expect(values_for(grid.box.first)).to eq [0,1,5,0,0,0,2,7,0]
  end

  it "should know what is in the last box " do
    cells = grid.box.last.map  {|c| c.value}
    expect(cells).to eq [0, 4, 0, 0, 2, 5, 6, 0, 0]
  end
  
  it "they are all zeroes" do
    cells = grid.each_empty_cell.map{|c| c.value}.uniq
    expect(cells).to eq [0]
  end

  it "should index the cells"
